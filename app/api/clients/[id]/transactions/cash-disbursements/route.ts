...
import { postCashDisbursement } from "@/lib/data/post-transaction";
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const entryId = await postCashDisbursement(auth.user.id, {
      clientId,
      contactId: parsed.data.contactId || undefined,
      disbursementDate: parsed.data.date,
    ...
    return NextResponse.json(
      { error: err instanceof Error ? err.message : "Failed to post cash disbursement." },
      { status: 400 }
    );
  }
}
